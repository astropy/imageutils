...
def run(np.ndarray[np.float32_t, ndim=2, mode='c', cast=True] indat,
                      np.ndarray[np.uint8_t, ndim=2, mode='c', cast=True] inmask=None,
                       ...
                       int niter=4, fullmedian=False, cleantype='meanmask', finestructuremode='median', psfmodel='gauss', psffwhm = 2.5, psfsize = 7, psfk = None,psfbeta = 4.765 ):
    ...
    for i in range(niter):
        ...
        # Go through and clean the image using a masked mean filter
            ...
            raise ValueError('cleantype must be one of the following values: [median, meanmask, medmask, idw]')
...
@cython.wraparound(False)
@cython.cdivision(True)
cdef void clean_meanmask(float[:, ::1] cleanarr, bool[:, ::1] crmask, bool[:,::1] mask, int nx, int ny,
...
@cython.wraparound(False)
@cython.cdivision(True)
cdef void clean_medmask(float[:, ::1] cleanarr, bool[:, ::1] crmask, bool[:,::1] mask, int nx, int ny,
    ...
    with nogil, parallel():
        ...
        for j in prange(2, ny - 2):
            for i in range(2, nx - 2):
                ...
                if crmask[j, i]:
                    numpix = 0
                    # median the 25 pixels around the pixel ignoring any pixels that are masked
                    for l in range(-2, 3):
                        for k in range(-2, 3):
                            if not (crmask[j + l, i + k] or mask[j + l, i + k]):
...
@cython.wraparound(False)
@cython.cdivision(True)                  
cdef void clean_idwinterp(float[:, ::1] cleanarr, bool[:, ::1] crmask, bool[:,::1] mask, int nx, int ny,
        float backgroundlevel):
    ...
    weightsarr = np.array([[ 0.35355339,  0.4472136 ,  0.5       ,  0.4472136 ,  0.35355339],
                          ...
                          [ 0.5       ,  1.        ,  0.        ,  1.        ,  0.5       ],
                          [ 0.4472136 ,  0.70710678,  1.        ,  0.70710678,  0.4472136 ],
                          [ 0.35355339,  0.4472136 ,  0.5       ,  0.4472136 ,  0.35355339]], dtype = np.float32)
            ...
            for i in range(2, nx - 2):
                ...
                if crmask[j, i]:
                    ...
                    for l in range(-2,3):
                        for k in range(-2,3):
                            if not (crmask[j + l, i + k] or mask[j + l, i + k]): 
                                val = val + weights[l,k]*cleanarr[j+l,i+k]
